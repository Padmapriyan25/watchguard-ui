...
function EmptyDashboardTab({ activeTab }: { activeTab: 'Threats' | 'Licenses' }) {
  ...
  return (
    <div className="px-4 py-4 sm:px-5 sm:py-5 lg:px-6">
      <article className="rounded-[20px] border border-[#dde5ec] bg-gradient-to-br from-white to-[#f7fafc] p-5 shadow-[0_14px_32px_rgba(15,23,42,0.05)] sm:p-8">
        <div className="flex max-w-2xl items-start gap-4">
          ...
          <div>
            ...
            <h2 className="mt-2 text-[1.05rem] font-semibold tracking-[-0.03em] text-[#1f2937] sm:text-[1.35rem]">
              {placeholder.title}
            </h2>
...
export default function DashboardPage() {
  ...
  return (
    <section className="w-full px-0">
      <div className="border-x border-b border-[#efc9c9] bg-[#f7d7d7]">
        <div className="flex items-start justify-between gap-3 px-4 py-3 text-[12px] text-[#7f4444] shadow-[inset_0_-1px_0_rgba(255,255,255,0.35)] sm:items-center sm:px-5 lg:px-6">
          <div className="flex min-w-0 items-start gap-3 sm:items-center">
            <AlertCircle className="h-4 w-4 shrink-0" />
            <p className="font-medium leading-5">{dashboardPageData.alertMessage}</p>
          </div>
          <ChevronRight className="mt-0.5 h-4 w-4 shrink-0 sm:mt-0" />
        </div>
      </div>

      <div className="overflow-hidden rounded-b-[22px] border border-t-0 border-[#dde4ea] bg-white shadow-[0_16px_30px_rgba(15,23,42,0.04)]">
        <div className="flex flex-col items-start justify-between gap-4 px-4 pt-5 pb-4 sm:px-5 md:flex-row lg:px-6 lg:pt-6">
          <HeroHeader />
          <div className="flex items-center gap-2 text-[12px] text-[#59666f] md:pt-2">
        ...
        {activeTab === 'Home' ? (
          <div className="px-4 py-4 sm:px-5 sm:py-5 lg:px-6">
            <div className="grid items-start gap-3 xl:grid-cols-[minmax(0,1fr)_275px]">
              ...
              <div className="grid gap-3 self-start lg:grid-cols-2 xl:grid-cols-1">
                <ProductPanel />
                <ActionCards />
              </div>
            </div>
